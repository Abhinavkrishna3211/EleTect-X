// EleTect X — reviewer demo accounts.
//
// Reviewers need a working login to evaluate the dashboard, but the login page must
...
\`.git/info/exclude\`. Send them to reviewers directly (email or a
private channel).
...
Rotate or delete these credentials once they are no longer needed.
