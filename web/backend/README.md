...
- **Demo fallback if DLT isn't ready:** switch `SMS_PROVIDER` to Twilio trial (verified numbers only) or send **email** alerts (Supabase/Resend free) to prove the pipeline; swap to production SMS when DLT clears.
