...
active) and Workstream 2 (the Boar representation audit, still not started as of this checkpoint).
...
**Not yet done as of this checkpoint:** none of this session's work is committed yet — `camera.py`,
`video.py`, `config.py`, `reflex_loop.py` plus the three test files and three doc files are all
modified but uncommitted. Workstream 2 (`ml/vision/boar-representation-audit.md` — per-source Boar
breakdown, the 0.87% real night/IR figure, SA-FARI as lead sourcing candidate with its two named
open blocking checks) has not been started.
