...
- `docs/design-reference/EleTect Platform.dc.html` — the finished design, pixel spec for the 8 public pages + dashboard shell. Match it, don't reinterpret it.
- `docs/design-reference/SectorMap.dc.html` — a **dashboard component** (corridor/map view), out of scope until the dashboard-modules phase (build order step 5). Do not wire it into the public pages or pull it in early.
...
- Repo-wide: no AI/assistant references in code, comments, commits, PR descriptions, or docs. Write every commit message as a human engineer describing their own work — no "generated by," "co-authored by Claude," tool attribution, or emoji decoration.
- Conventional Commits, one logical unit per commit (e.g., shared layout/nav/footer as one commit, then one commit per page — not one giant commit for all of Phase 2). History should read like a senior engineer's clean, deliberate build-up, not a batch dump.
- Before marking a page done: it must visually match `docs/design-reference/EleTect Platform.dc.html` for that page — same layout, copy, spacing intent — not just "close enough."
