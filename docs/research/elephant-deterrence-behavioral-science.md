...
as "proven effective for elephants" anywhere in public/DFO materials — none of the literature supports a
...
evidenced alternative, if this comes up in DFO conversation.
...
within days-to-weeks" line, as the lead citation in public/DFO materials going forward.
...
- **No Hz number is defensible as "proven for elephants"** — keep `PATTERN_FAST_STROBE`'s ~6-8Hz framed
  internally as an engineering choice, not a cited optimum, in any doc that goes external (public/DFO).
...
- **Public-facing claims should cite Adams et al. 2020 as the lead source**, not the currently-uncited
  ...
  or external reviewer could reasonably ask for a source).
...
### 7.8 What to say to DFO, in one paragraph
