...
The reference implementation busy-waits near 100% CPU to keep the SAI FIFO from overrunning, which is
...
- **Drop the analog mic entirely; run INMP441 continuously via SAI.** Rejected, unchanged from ADR 0006 —
  The reference implementation's own near-100%-busy finding makes this incompatible with the µA-sleep
