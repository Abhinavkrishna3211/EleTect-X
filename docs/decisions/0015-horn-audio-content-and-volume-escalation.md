...
**Attribution is a real, non-optional obligation for files 2 and 3** if this ships in a public/contest/DFO
...
- **Reuse D0/D1 (the LoRa UART) with time-division sharing.** Rejected: `PIN_MAP.md` already marks that
  ...
  troubled bring-up is a worse starting position than two new dedicated pins on an uncontested link.
...
- Attribution obligations (files 2 and 3, CC-BY) are a real, tracked-here requirement on whatever
  contest/DFO-facing materials ship with these tracks — not automatically satisfied by this ADR existing.
