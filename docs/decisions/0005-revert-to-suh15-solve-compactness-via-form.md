...
cost premium at prototype scale. On reflection, prompted by a direct question about schedule impact, that
trade was wrong for two reasons stronger than "does it cost presentation points":

1. **Neither published rubric scores cost or scalability directly.**
   The scoring criteria used for this kind of build don't weight cost or scalability on their own
   — so the swap bought nothing on that axis. But Technical/Project
   ...
   aesthetics, sitting next to that stated principle, is a self-inconsistency a careful reviewer could
   reasonably notice.
2. **This isn't just a prototype artifact — it's the actual deployment model.** CONTEXT.md's own spacing is
   nodes every 120–150m along a forest boundary, and the mission's priority order puts real DFO field
   deployment above any short-term scoring. At even 50 nodes, the SC-610 swap is roughly ₹375,000 in extra
...
- **Split the difference (TOA SC-610 for the demo unit, SUH-15 documented as the production
